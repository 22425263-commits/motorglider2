...
# AÑADIR AQUÍ - Después de esta línea anterior

# ╔═╡ NEW: Rate of Climb Input
md" **Available Engine Power** (W) = $(@bind Power_available NumberField(0:1000:10000000, default=50000)) "

# ╔═╡ NEW: Rate of Climb Functions
begin
    Climb_Rate(TAS, h, CD0, Sw, e, W, AR, Power_available) = (Power_available - Power_required(TAS, h, CD0, Sw, e, W, AR)) / W
    
    function Best_Rate_Climb_Speed(CD0, Sw, e, W, AR, h, Power_available, CLmax_val, Stall_TAS)
        v_search = LinRange(Stall_TAS, 150, 200)
        climb_rates = Climb_Rate.(v_search, h, CD0, Sw, e, W, AR, Power_available)
        _, idx = findmax(climb_rates)
        return v_search[idx]
    end
    
    function Best_Angle_Climb_Speed(CD0, Sw, e, W, AR, h, Power_available, CLmax_val, Stall_TAS)
        v_search = LinRange(Stall_TAS, 150, 200)
        climb_angles = Climb_Rate.(v_search, h, CD0, Sw, e, W, AR, Power_available) ./ v_search
        _, idx = findmax(climb_angles)
        return v_search[idx]
    end
end;

# ╔═╡ NEW: Calculate climb speeds
begin
    Vy = Best_Rate_Climb_Speed(CD0, Sw, Oswald, Mass*g(), AR, Alt_op, Power_available, CLmax, TASstall)
    Vx = Best_Angle_Climb_Speed(CD0, Sw, Oswald, Mass*g(), AR, Alt_op, Power_available, CLmax, TASstall)
    
    RC_max = Climb_Rate(Vy, Alt_op, CD0, Sw, Oswald, Mass*g(), AR, Power_available)
    RC_at_Vx = Climb_Rate(Vx, Alt_op, CD0, Sw, Oswald, Mass*g(), AR, Power_available)
end;

# ╔═╡ NEW: Display climb performance
md"""
**Vy (Best Rate of Climb Speed)**: $(round(Vy; digits=1)) m/s · $(round(ms2kt(Vy); digits=1)) kt · RC max = $(round(RC_max; digits=2)) m/s

**Vx (Best Angle of Climb Speed)**: $(round(Vx; digits=1)) m/s · $(round(ms2kt(Vx); digits=1)) kt · RC = $(round(RC_at_Vx; digits=2)) m/s
"""

# ╔═╡ NEW: Plot Rate of Climb
begin
    v4 = LinRange(TASstall, max_TAS_plot, 150)
    rc_values = Climb_Rate.(v4, Alt_op, CD0, Sw, Oswald, Mass*g(), AR, Power_available)
    ymax_rc = 1.2 * maximum(rc_values)
    
    plot(xlims=(TASstall, max_TAS_plot), ylims=(minimum(rc_values)-0.5, ymax_rc), xticks=x_tick_values, leg=true, size=(660, 400), grid=(:xy, :olivedrab, :dot, 0.5, 0.8), xrotation=45, xlabel="TAS (m/s)", ylabel="Rate of Climb (m/s)", title="Rate of Climb vs TAS at $(Alt_op)m")
    plot!(v4, rc_values, label="Rate of Climb", linewidth=3, color=:blue)
    scatter!([Vy], [RC_max], label="Vy", ms=6, color=:green)
    scatter!([Vx], [RC_at_Vx], label="Vx", ms=6, color=:red)
    annotate!([Vy], [RC_max*1.1], text("Vy\n$(round(Vy;digits=1))m/s", 8, :green, :center))
    annotate!([Vx], [RC_at_Vx*0.85], text("Vx\n$(round(Vx;digits=1))m/s", 8, :red, :center))
    plot!(legend=:topright)
end

# ╔═╡ NEW: Plot Climb Angle
begin
    climb_angles = similar(rc_values)
    for i in eachindex(rc_values)
        if v4[i] > 0 && rc_values[i] > 0
            climb_angles[i] = asin(min(rc_values[i]/v4[i], 1.0)) * 180 / π
        else
            climb_angles[i] = 0
        end
    end
    ymax_angle = 1.2 * maximum(climb_angles)
    
    plot(xlims=(TASstall, max_TAS_plot), ylims=(0, ymax_angle), xticks=x_tick_values, leg=true, size=(660, 400), grid=(:xy, :olivedrab, :dot, 0.5, 0.8), xrotation=45, xlabel="TAS (m/s)", ylabel="Climb Angle (°)", title="Climb Angle vs TAS at $(Alt_op)m")
    plot!(v4, climb_angles, label="Climb Angle", linewidth=3, color=:purple)
    scatter!([Vx], [asin(min(RC_at_Vx/Vx, 1.0))*180/π], label="Vx", ms=6, color=:red)
    annotate!([Vx], [asin(min(RC_at_Vx/Vx, 1.0))*180/π*1.15], text("Vx", 8, :red, :center))
    plot!(legend=:topright)
end
